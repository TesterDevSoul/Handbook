...
用来设定等待时间，可以定义每个请求之间的等待时间。当在网站或应用程序上执行任何操作时，自然会有停顿和延迟，这些都可以用定时器模拟。

>比如：等待5秒后再发送请求；<br>让多个请求等待到同一时刻发送。

不同定时器有不同功能。JMeter 发送请求时不会在每个取样器/请求之间有任何的延迟。

如果所有取样器的请求都立即发送给服务器，此时将会超载，这不是我们模拟压测时想要的。这时可以添加一个**计时器**元件，它**定义每个请求之间的等待时间**。
...
性能测试就是通过模拟大量用户负载的情况，来验证服务器端的性能。那线程组就是用来完成这个工作的，在线程组原件中我们可以设置运行的线程数即模拟用户数，一个线程就相当于一个用户。线程组元件出了可以设置线程数以外，还可以设置对应的运行的时长及定时运行等。具体的元件使用讲解会在后面给大家详细介绍。
...
![](https://cdn.jsdelivr.net/gh/TesterDevSoul/pic/manual/20230220143954.png)